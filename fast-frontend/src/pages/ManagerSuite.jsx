...
// ── Map click handler for zone drawing ────────────────────────────────────────
...
export default function ManagerSuite() {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      setUserOk(true);
      setTimeout(() => setUserOk(false), 2500);
      fetchUsers();
  ...
  const handleMapPoint = (pt) => {
    if (!corner1) {
      ...
      setPendingZone({
        minLat: Math.min(corner1[0], pt[0]), maxLat: Math.max(corner1[0], pt[0]),
        minLon: Math.min(corner1[1], pt[1]), maxLon: Math.max(corner1[1], pt[1]),
      });
      setCorner1(null);
      setDrawMode(false);
    }
  ...
  const handleSaveZone = async () => {
    ...
    try {
      ...
      setPendingZone(null); setZoneName('');
      fetchZones();
  ...
  const cancelDraw = () => {
    ...
    setPendingZone(null); setZoneName(''); setZoneError('');
  };

  const handleDeleteZone = async (id) => {
    await axios.delete(`${API_BASE}/api/nogozones?id=${id}`).catch(() => {});
    fetchZones();
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', direction: 'rtl' }}>
      ...
      {/* ── ICON SIDEBAR ─────────────────────────────────────────────────── */}
      <div style={{ width: 60, background: '#1a1a2e', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 20, gap: 6, flexShrink: 0 }}>
        <div style={{ color: 'white', fontSize: 20, marginBottom: 12 }}>⚙️</div>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} title={t.label} style={{
            width: 42, height: 42, borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 20,
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#f8f9fb' }}>
        ...
        {/* Header */}
        <div style={{ background: 'white', borderBottom: '1px solid #e8eaed', padding: '13px 22px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <span style={{ fontSize: 18 }}>{TABS.find(t => t.id === tab)?.icon}</span>
          <div style={{ fontWeight: 700, fontSize: 16 }}>{TABS.find(t => t.id === tab)?.label}</div>
        </div>

        <div style={{ flex: 1, overflow: tab === 'zones' ? 'hidden' : 'auto', padding: tab === 'zones' ? 0 : 22 }}>
            ...
            <div style={{ display: 'flex', gap: 22, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1, ...s.card }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['שם תצוגה', 'שם משתמש', 'תפקיד', 'אמבולנס', 'פעולה'].map(h => (
                  ...
                  <tbody>
                    {users.map(u => (
                      ...
                      >
                        <td style={s.td}><b>{u.displayName}</b></td>
                        ...
                        <td style={s.td}>
                          <button onClick={() => handleDeleteUser(u.id)} style={s.deleteBtn}>מחק</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div style={{ width: 290, ...s.card, flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                  ...
                  {userOk    && <div style={{ color: '#27ae60', fontSize: 13, background: '#eafaf1', padding: '8px 10px', borderRadius: 8 }}>✓ המשתמש נוסף בהצלחה</div>}
                  <div>
          ...
          {tab === 'events' && (
            <div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 18, alignItems: 'center' }}>
                ...
                <button onClick={fetchCases} style={{ marginRight: 'auto', padding: '6px 14px', border: '1px solid #e0e0e0', borderRadius: 20, background: 'white', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit', color: '#666' }}>
                  🔄 רענן
                </button>
              </div>
                  ...
                  <div key={c.id} style={s.card}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}
                      ...
                      <div style={{ textAlign: 'left', flexShrink: 0 }}>
                        <div style={{ fontSize: 12, color: '#aaa' }}>{fmtTime(c.createdAt)}</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                          {c.description && (
                            <div style={s.detailBox}>
                              <div style={s.detailLabel}>תיאור האירוע</div>
                              <div>{c.description}</div>
                            </div>
                          )}
                          {c.patientDetails && (
                            <div style={s.detailBox}>
                              <div style={s.detailLabel}>פרטי מטופל</div>
                              <div>{c.patientDetails}</div>
                            </div>
                          )}
                        </div>
                        {c.notes ? (
                          <div>
                            <div style={{ ...s.detailLabel, marginBottom: 6 }}>📋 היסטוריית עדכוני מוקד לצוות</div>
          ...
          {tab === 'zones' && (
            <div style={{ display: 'flex', height: '100%' }}>
              ...
              {/* Zones sidebar */}
              <div style={{ width: 300, background: '#f8f9fb', borderLeft: '1px solid #e8eaed', overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 12, flexShrink: 0 }}>
                <div style={s.cardTitle}>אזורי איסור חריגה לנגד הכיוון</div>
                <div style={{ fontSize: 12, color: '#7a5900', lineHeight: 1.7, background: '#fff8e1', padding: '10px 12px', borderRadius: 10, border: '1px solid #ffe082' }}>
                  באזורים אלו, גם בנסיעת חירום האמבולנס מחויב לנסוע לפי חוקי התנועה הרגילים — ללא נסיעה נגד כיוון.
                </div>

                {!drawMode && !pendingZone && (
                  <button onClick={() => setDrawMode(true)} style={{ ...s.btn, background: '#1a1a2e', color: 'white', cursor: 'pointer' }}>
                    ✏️ סמן אזור חדש על המפה
                  </button>
                )}
                ...
                {drawMode && (
                  <div style={{ background: '#e3f2fd', border: '1px solid #90caf9', borderRadius: 10, padding: '12px 14px', fontSize: 13 }}>
                    <b style={{ color: '#1565c0' }}>
                      {corner1 ? '📍 לחץ על נקודה שנייה במפה' : '📍 לחץ על נקודה ראשונה במפה'}
                    </b>
                    <div style={{ marginTop: 10 }}>
                      <button onClick={cancelDraw} style={{ padding: '5px 14px', border: '1px solid #ccc', borderRadius: 8, background: 'white', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
                        ביטול
                      </button>
                    </div>
                  </div>
                ...
                {pendingZone && (
                  <div style={{ ...s.card, padding: 14 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8 }}>✅ אזור סומן — תן לו שם</div>
                    {zoneError && <div style={{ color: '#c0392b', fontSize: 12, marginBottom: 6 }}>⚠️ {zoneError}</div>}
                    <input
                      value={zoneName}
                      onChange={e => setZoneName(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && handleSaveZone()}
                      placeholder="לדוג׳: מרכז העיר"
                      style={{ ...s.input, marginBottom: 10 }}
                      autoFocus
                    />
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={handleSaveZone} style={{ flex: 1, padding: '9px 0', border: 'none', borderRadius: 9, background: '#ff3b30', color: 'white', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
                        💾 שמור
                      </button>
                      <button onClick={cancelDraw} style={{ flex: 1, padding: '9px 0', border: 'none', borderRadius: 9, background: '#e0e0e0', color: '#555', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
                        ביטול
                      </button>
                    </div>
                  </div>
                ...
                {zones.length === 0 && !drawMode && !pendingZone && (
                  <div style={{ color: '#ccc', textAlign: 'center', fontSize: 13, padding: 20 }}>אין אזורי איסור מוגדרים</div>
                )}
                  ...
                  <div key={z.id} style={{ background: 'white', border: '1px solid #ffd5d0', borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#aaa', marginTop: 2 }}>
                        {z.minLat.toFixed(4)}° – {z.maxLat.toFixed(4)}° N
                      </div>
                    </div>
              ...
              <div style={{ flex: 1 }}>
                <MapContainer
                  ...
                  style={{ height: '100%', width: '100%', cursor: (drawMode || corner1) ? 'crosshair' : 'grab' }}
                >
                  <TileLayer
                  ...
                  {pendingZone && (
                    <Rectangle
                      bounds={[[pendingZone.minLat, pendingZone.minLon], [pendingZone.maxLat, pendingZone.maxLon]]}
                      color="#ff9500" weight={2} fillOpacity={0.2} dashArray="6 4"
                    />
                  )}
...
// ── Styles ────────────────────────────────────────────────────────────────────
const s = {
  card:        { background: 'white', borderRadius: 12, padding: '16px 20px', border: '1px solid #e8eaed', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' },
  cardTitle:   { fontWeight: 700, fontSize: 15, marginBottom: 14, color: '#1a1a2e' },
  th:          { textAlign: 'right', padding: '8px 14px', fontSize: 12, color: '#aaa', fontWeight: 600, borderBottom: '2px solid #f0f0f0', whiteSpace: 'nowrap' },
  td:          { padding: '11px 14px', borderBottom: '1px solid #f5f5f5', fontSize: 14, verticalAlign: 'middle' },
  ...
  input:       { width: '100%', padding: '9px 11px', borderRadius: 9, border: '1px solid #e8eaed', fontSize: 14, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box', direction: 'rtl', background: 'white' },
  btn:         { width: '100%', padding: '11px 0', border: 'none', borderRadius: 9, fontWeight: 700, fontSize: 14, fontFamily: 'inherit' },
